...
public class SimulatorHelper {
    ...
    final static Logger logger = LoggerFactory.getLogger(SimulatorHelper.class);
    ...
    {
        logger.info("start creating simulated Order...");
        synchronized (createOrderMutex) {
            try {
                ...
                int iTickerChoice = (int) ((Math.random() * 100) % 3);
                ...
                logger.info("Created simulated Order with ID " + bldr.getOrderId());
                return bldr.build();
            }
            catch (Exception e)
            {
                logger.error("Exception caught creating simulated Order.",e);
            }
            return null;
        }
    }
    ...
    {
        logger.info("start creating simulated Execution...");
        synchronized (fillOrderMutex) {
            try
            {
                ...
                logger.info("Created simulated Order with ID " + bldr.getExecId());
                return bldr.build();
            }
            catch (Exception e)
            {
                logger.error("Exception caught creating simulated Execution.",e);
            }
            return null;
        }
    }
